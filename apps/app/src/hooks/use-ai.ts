...
/** One generation, polled (every 2s by default) while the worker is on it. */
export function useGeneration(id: string | null, pollMs = 2_000) {
	const { orgId } = useOrg();
	return useQuery({
		queryKey: qk.generation(orgId, id ?? ""),
		queryFn: () => call(api.ai.generations[":id"].$get({ param: { id: id ?? "" } })),
		enabled: Boolean(id),
		refetchInterval: (q) => (isGenerationActive(q.state.data) ? pollMs : false),
	});
}
...
/**
 * Starts an async generation (image, carousel, video) and follows it to completion:
 * seeds the poll with the 202 body, then refreshes budget, history and the
 * media library once the worker is done.
 */
export function useGenerationRun<TInput>(
	start: (input: TInput) => Promise<Generation>,
	...
	const generation = useGeneration(id, pollMs);
